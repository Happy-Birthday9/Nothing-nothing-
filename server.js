...
app.get("/api/logo", async (req, res) => {
    try {
        const page = await fetch("https://ibb.co.com/wrNzwV5Y", {
        ...
        const match =
            ...
            html.match(/https?:\/\/i\.ibb\.co\/[^"'\\s<>]+/i);

        if (!match) throw new Error("Logo image URL not found");
        ...
        // Tiny transparent PNG fallback if ImgBB is temporarily unavailable.
